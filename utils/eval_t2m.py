...
def evaluation_mask_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div,
    ...
    diversity_real = calculate_diversity(motion_annotation_np, 300 if nb_sample > 300 else 1)
    diversity = calculate_diversity(motion_pred_np, 300 if nb_sample > 300 else 1)
...
@torch.no_grad()        
def evaluation_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, plot_func, save_ckpt = True, save_anim=True) : 
                        #    out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, plot_func, save_ckpt=False, save_anim=True
    trans.eval()
    nb_sample = 0
    ...
    for i in range(1):
        ...
        for batch in val_loader:
            ...
            # pred_poses = trans.generate(clip_text, m_length)
            for k in range(bs):
                pred_pose = trans.generate(clip_text[k:k+1], m_length, motion_ids[k:k+1])
                # print(pred_pose)
                # try:
                #     all_pred_motions = trans.generate(clip_text, m_length)
                # except:
            ...
            print(f"pred_pose_eval===>{pred_pose_eval.shape}")
...
@torch.no_grad()
def evaluation_transformotion(val_loader, vq_model, trans, repeat_id, eval_wrapper,
                                time_steps, temperature, topkr, force_mask=False,
                                              cal_mm=True):
    trans.eval()
    vq_model.eval()
    ...
    for i, batch in enumerate(val_loader):
        ...
        bs, seq = pose.shape[:2]
        # num_joints = 21 if pose.shape[-1] == 251 else 22
        print(f"clip_textclip_text===>{len(clip_text)}")
        # l = torch.full((1, len(clip_text)), 60)
        # for i in range(mm_batch)
        if i < num_mm_batch:
        # (b, seqlen, c)
            motion_multimodality_batch = []
            # labels = torch.zeros(len(clip_text), l, dtype=torch.long)
            for _ in range(30):
                pred_motions = trans.generate(clip_text, m_length)
            ...
            pred_motions = trans.generate(clip_text, m_length, labels)
